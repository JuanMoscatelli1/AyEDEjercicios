...
void CargarAlumnosPrueba(void)
{
    ...
    if (f)
    {
        ...
        strcpy(x.nombre,"GUILLERMO");
        strcpy(x.dni,"35678321");
        x.codcarrera=10;
        x.legajo=117764;
        fwrite(&x, sizeof(Alumno),1,f);

        strcpy(x.apellido,"RUIZ");
        strcpy(x.nombre,"KAREN");
        strcpy(x.dni,"36432128");
        x.codcarrera=10;
        x.legajo=126211;
        fwrite(&x, sizeof(Alumno),1,f);

        strcpy(x.apellido,"CHRISTIAN");
        strcpy(x.nombre,"DIOR");
...
void levantarArchivos(NodoListaSEAlumno *&a)
{
    ...
    while (fread(&m,sizeof(Materia),1,f))
    {
        ...
        if (aux) //Por si no se econtró el alumno
